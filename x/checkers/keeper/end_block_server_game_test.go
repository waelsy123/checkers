...
func TestForfeit2OldestPlayedOnceIn1Call(t *testing.T) {
	...
	paywael := escrow.ExpectPayWithDenom(context, wael, 46, "coin").Times(1).After(payBob)
	refundBob := escrow.ExpectRefund(context, bob, 45).Times(1).After(paywael)
	escrow.ExpectRefundWithDenom(context, wael, 46, "coin").Times(1).After(refundBob)
	msgServer.PlayMove(context, &types.MsgPlayMove{
		Creator:   bob,
		GameIndex: "1",
		FromX:     1,
...
func TestForfeitPlayedTwice(t *testing.T) {
	...
	defer ctrl.Finish()
	payBob := escrow.ExpectPay(context, bob, 45).Times(1)
	paywael := escrow.ExpectPay(context, wael, 45).Times(1).After(payBob)
	escrow.ExpectRefund(context, wael, 90).Times(1).After(paywael)
	msgServer.PlayMove(context, &types.MsgPlayMove{
		Creator:   bob,
		GameIndex: "1",
		FromX:     1,
...
func TestForfeit2OldestPlayedTwiceIn1Call(t *testing.T) {
	...
	defer ctrl.Finish()
	payBob := escrow.ExpectPay(context, bob, 45).Times(1)
	paywael1 := escrow.ExpectPay(context, wael, 45).Times(1).After(payBob)
	paywael2 := escrow.ExpectPayWithDenom(context, wael, 46, "coin").Times(1).After(paywael1)
	payAlice := escrow.ExpectPayWithDenom(context, alice, 46, "coin").Times(1).After(paywael2)
	refundwael := escrow.ExpectRefund(context, wael, 90).Times(1).After(payAlice)
	escrow.ExpectRefundWithDenom(context, alice, 92, "coin").Times(1).After(refundwael)
	msgServer.PlayMove(context, &types.MsgPlayMove{
		Creator:   bob,
		GameIndex: "1",
		FromX:     1,
